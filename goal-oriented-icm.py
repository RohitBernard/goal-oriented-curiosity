...
class ActorAgent(object):
    def __init__(
            ...
            use_cuda=False,
            use_noisy_net=True):
        self.model = GoalOrientedCnnActorCriticNetwork(
            input_size, output_size, goal_size, use_noisy_net)
    ...
    def forward_transition(self, state, next_state, goal):
        state = torch.from_numpy(state).to(self.device)
        state = state.float()
        goal = torch.Tensor(goal).to(self.device)
        policy, value = agent.model(state, goal)

        next_state = torch.from_numpy(next_state).to(self.device)
        next_state = next_state.float()
        _, next_value = agent.model(next_state, goal)
    ...
    def train_model(
        ...
        with torch.no_grad():
            ...
            policy_old, value_old = self.model(s_batch, goal)
        ...
        for i in range(epoch):
            ...
            for j in range(int(len(s_batch) / batch_size)):
                ...
                policy, value = self.model(s_batch[sample_idx], goal)
...
def goal_encoder(name):
    if name == "VizdoomCvPathfindersBlue-v0":
        return [0,0,1]
    elif name == "VizdoomCvPathfindersRed-v0":
        return [0,1,0]
    elif name =="VizdoomCvPathfindersYellow-v0":
        return [1,0,0]
    else:
        return [0,0,0]

if __name__ == '__main__':
    env_id = "VizdoomCvPathfindersBlue-v0"
    movement = COMPLEX_MOVEMENT
    # env = JoypadSpace(
    #     gym_super_mario_bros.make(env_id), movement)
    env = gym.make("VizdoomCvPathfindersBlue-v0")
    input_size = env.observation_space['screen'].shape  # 4
    ...
    goal_env_ids = ["VizdoomCvPathfindersBlue-v0", "VizdoomCvPathfindersRed-v0", "VizdoomCvPathfindersYellow-v0"]
    goals = []
    works = []
    parent_conns = []
    child_conns = []
    for idx in range(num_worker):
        ...
        work = MarioEnvironment(goal_env_ids[idx%3], is_render, idx, child_conn)
        work.start()
        works.append(work)
        parent_conns.append(parent_conn)
        child_conns.append(child_conn)
        goals.append(goal_encoder(goal_env_ids[idx%3]))
    ...
    while True:
        ...
        for _ in range(num_step):
            ...
            actions = agent.get_action(states,goals)
        ...
        if is_training:
            ...
            value, next_value, policy = agent.forward_transition(
                total_state, total_next_state, goals)
